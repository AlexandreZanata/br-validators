...
| **RG (Registro Geral)** | Per-state SSP/IGP | Full table: [§ RG](#rg--reference-index) | **UF required** — no federal algorithm. **12/27 UFs:** SP, RJ, MG, PR, RS, SC, BA, AC, AL, AM, AP, DF. SP/RJ/MG: Ghiorzi DV tables; PR/RS/BA/AC/AL/AM/AP/DF: format-only; SC: format-only + mask. Vectors: `rg.{sp,rj,mg,pr,rs,sc,ba,ac,al,am,ap,df}.official.json`. |
...
> **Caveat:** No single federal RG algorithm — each state issues its own format. `validateRg(raw, { uf })` requires UF; `detect()` does **not** auto-classify RG (too ambiguous without UF). Remaining 15 UFs: community contributions via [rg-uf-contribution issue template](../.github/ISSUE_TEMPLATE/rg-uf-contribution.md).
