...
| **RG (Registro Geral)** | Per-state SSP/IGP | Full table: [§ RG](#rg--reference-index) | **UF required** — no federal algorithm. **8/27 UFs:** SP, RJ, MG, PR, RS, SC, BA, AC. SP/RJ/MG: Ghiorzi DV tables; PR/RS/BA/AC: format-only; SC: format-only + mask. Vectors: `rg.{sp,rj,mg,pr,rs,sc,ba,ac}.official.json`. |
...
| **AL** | ⏳ Community | Research required — do not reuse SEFAZ-AL IE calculator | [PCivil AL](https://www.policiacivil.al.gov.br/) | — |
